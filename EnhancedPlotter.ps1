...
$tempPlotSize = 250 #Size that the temp files for one k32 plot take (Currently ~260GiB/240GB as of v1.1.3)
        ...
        if ($freeMemory -gt $($lowMemThreshold + $memoryBuffer)) {
            ...
            if ($CPUAvg -lt $lowCpuThreshold) {
                ...
                $plots | Where-Object {$_.tempDrive -like $tempDrive} | foreach {
                    if ($(Get-Process -ErrorAction 'SilentlyContinue' -Id $($_.procId)) -ne $null) {
                        $runningPlots++
                    }
                }
                Write-Host "$runningPlots plots running on $tempDrive"
                $reservedSpace = $runningPlots * $tempPlotSize*1024*1024*1024
                ...
                if ($tempSpace.FreeSpace -gt $($tempPlotSize*1024*1024*1024 + $reservedSpace)){
                    Write-Host "Free space on $tempDrive $([math]::Round($tempSpace.FreeSpace/1024/1024)) GB is large enough to hold new $tempPlotSize GiB plot + $($reservedSpace/1024/1024/1024) GiB running plotting processes. Continuing... "
                ...
                else { Write-Host "$tempDrive has insufficient space. Only $([math]::Round($tempSpace.FreeSpace/1024/1024/1024))GiB remaining. Waiting $delayBetweenChecks seconds..."; sleep $delayBetweenChecks }
            }
