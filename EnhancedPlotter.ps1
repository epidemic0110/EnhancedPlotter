        ...
        if ($freeMemory -gt $($lowMemThreshold + $memoryBuffer)) {
            ...
            if ($CPUAvg -lt $lowCpuThreshold) {
                ...
                if ($tempSpace.FreeSpace -gt $($tempPlotSize*1024*1024*1024 + $reservedSpace)){
                    Write-Host "Free space on $tempDrive $([math]::Round($tempSpace.FreeSpace/1024/1024)) GiB is large enough to hold new $tempPlotSize GiB plot + $($reservedSpace/1024/1024/1024) GiB running plotting processes. Continuing... "
